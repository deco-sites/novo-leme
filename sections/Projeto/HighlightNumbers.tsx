...
export default function HighlightNumbers({
  ...
  return (
    <div className="container flex gap-16 mb-9">
      ...
      <div className={`w-full grid grid-cols-1 gap-6 justify-stretch ${highlights?.length === 1 ? 'md:px-60' : ''} ${highlights?.length % 2 === 0 ? 'md:grid-cols-2 md:px-24' : ''} ${highlights?.length % 3 === 0 ? 'md:grid-cols-3 md:px-12' : ''}`}>
        {highlights?.map((item) => (
          <div className={`
